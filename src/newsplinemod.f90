...
! newspline, a fast, means-preserving spline for interval data
! Leo O Lai and Jed O. Kaplan, 2020
! Lai, L. O., & Kaplan, J. O. (2022). A fast mean-preserving spline for interpolating interval data. 
! Journal of Atmospheric and Oceanic Technology, 39(4), 503-512. doi:10.1175/JTECH-D-21-0154.1

use parametersmod, only : sp
use utilitymod,    only : pos,matsol !,findloc

implicit none

public  :: newspline            ! Wrapper subroutine with all optional arguments
private :: newspline_noarg      ! Subroutine of newspline base function with no optional arguments
...
subroutine newspline(monthdata,nk,daydata,llim,ulim,plim,prec)

! COMPOSITE WRAPPER SUBROUTINE : newspline
! ---
! newspline(monthdata, nk, daydata, llim, ulim, plim, prec)
! ---
...
integer :: precision
integer, dimension(:), allocatable :: int_daydata
...
if (present(plim)) then
  ...
  if (present(llim)) then
    ...
    if (present(ulim)) then
      ...
      call newspline_noarg(monthdata, nk, daydata)
...
subroutine newspline_noarg(monthdata,nk,daydata)
...
end subroutine newspline_noarg
...
do i = 2, (len-1)
  ...
  if (osc_check(i) ) then
    ...
    slpe_l = pos(x_new, (real(2*i)-root(i)))
