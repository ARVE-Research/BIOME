...
subroutine soilproperties(terrain,soilcoords,soilinput,soilstate)

use parametersmod,   only : sp
use typesmod,        only : terraintype,soilcoordstype,soilinputtype,soilstatetype
...
type(soilinputtype), dimension(:), intent(in)  :: soilinput
type(soilstatetype), dimension(:), intent(out) :: soilstate
...
nl = size(soilinput%sand)

usda = terrain%USDA

do l = 1,nl

  zpos = soilcoords(l)%zpos
  sand = soilinput(l)%sand
  clay = soilinput(l)%clay
  orgm = soilinput(l)%soc * omcf   ! convert soil organic carbon to organic matter
  cfvo = soilinput(l)%cfvo
  ...
  Tsat = fTsat(Dp,Db) * svol
    
  T33 = fT33(Tsat,clay,sand,orgm) * svol
...
! function to estimate soil porosity Theta-sat (Sandoval et al., 2024, eqn A25c) (fraction)
...
! function to estimate soil water content at field capacity Theta-33 (Sandoval et al., 2024, eqn A25b) (fraction)
...
! function to estimate soil water content at wilting point Theta-1500 (Sandoval et al., 2024, eqn A25a) (fraction)
