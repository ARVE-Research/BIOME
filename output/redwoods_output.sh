...
infile=${1}
output=redwoods_output.ps

title="Model Output: Redwoods National Park, Humboldt County, CA"
...
gmt psbasemap -R$t0/$t1/$tmin/$tmax -JX19/8 -Bpxa1O -Bpya10f2+l"Temperature (C)" -BWSn+t"$title" -X4 -Y20 -P -K > $output
...
gmt pslegend -R -J -DjTL+w3c+o0.2c -F+p1p+gwhite -O -P -K >> $output << EOF
...
gmt psbasemap -R$t0/$t1/0/$pet_max -JX19/ -Bpxa1O -Bpya+l"mm" -BWSen+t"Actual and Potential Evapotranspiration" -Y-8 -P -O -K >> $output
...
# legend
gmt pslegend -R -J -DjTL+w3.5c+o0.2c -F+p1p+gwhite -O -P -K >> $output << EOF
...
# legend
gmt pslegend -R -J -DjBL+w4c+o0.2c -F+p0.5p+gwhite -O -P << EOF >> $output
...
echo "Plotting complete. PDF saved as redwoods_output.pdf"
